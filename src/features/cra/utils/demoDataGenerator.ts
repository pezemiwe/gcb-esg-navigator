...
// Generate realistic exposure amounts
...
// Generate interest rate based on sector and tenor
function generateInterestRate(sector: string, tenor: number): number {
  ...
  // Longer tenor = slightly higher rate
...
// Generate demo assets with realistic data
export function generateDemoAssets(count: number = 500): Asset[] {
  ...
  const missingRegionProbability = 0.023; // 2.3%
  const invalidDateProbability = 0.008; // 0.8%
  const missingSectorProbability = 0.015; // 1.5%
  ...
  for (let i = 0; i < count; i++) {
    ...
    const tenor = Math.floor(Math.random() * 84) + 12; // 12-96 months
    ...
    assets.push({
      ...
      borrowerName: borrowerName + (i % 10 === 0 ? ` ${i}` : ""), // Add variation
  ...
  // Add a few duplicates (intentional data quality issue)
  for (let i = 0; i < 7; i++) {
    const randomIndex = Math.floor(Math.random() * assets.length);
    assets.push({ ...assets[randomIndex], id: assets[randomIndex].id }); // Same ID = duplicate
  }

  return assets;
}

// Generate demo data and populate store
export function populateDemoData(
  ...
  // Group by asset type for the store structure
